...
export function MobileNavigation(){
 ...
 return <div className="lg:hidden">
  ...
  <aside className={`${open?'translate-x-0':'-translate-x-full'} fixed inset-y-0 left-0 z-[95] w-[280px] bg-[#0f5b32] text-white p-5 pt-20 transition-transform duration-200 shadow-2xl`}><div className="pb-5 mb-4 border-b border-white/15"><Link href="/dashboard" onClick={()=>setOpen(false)} className="zeepay-wordmark zeepay-wordmark-light">Zee<span>Pay</span></Link><p className="text-[11px] text-white/60 mt-2">Your savings, payments & wallet</p></div><nav className="space-y-1 text-sm font-medium">{nav.map(([icon,href,label])=>{const active=pathname===href||(href!=='/dashboard'&&pathname.startsWith(`${href}/`));return <Link key={label} href={href} onClick={()=>setOpen(false)} aria-current={active?'page':undefined} className={`flex items-center gap-3 px-3 py-3 rounded-xl ${active?'bg-white/15 text-white font-semibold':'text-white/75 hover:bg-white/10 hover:text-white'}`}><span className="w-6 flex justify-center"><AlajoIcon name={icon} size={18}/></span>{label}</Link>})}</nav></aside>
 </div>
}
