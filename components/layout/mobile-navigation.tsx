...
export function MobileNavigation(){
 ...
 async function logout(){
  ...
  try{
   // Kill the browser session first so / cannot bounce back into the app.
   try{await createClient().auth.signOut({scope:'global'})}catch{}
   // Clear the server-side auth cookies as well. Do not make navigation depend on this request.
   fetch('/api/auth/logout',{method:'POST',credentials:'include',cache:'no-store',keepalive:true}).catch(()=>{})
  }finally{
   // Public Alajo home is the only logout destination on mobile.
