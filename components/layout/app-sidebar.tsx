...
export function AppSidebar() {
  ...
  useEffect(() => {
    fetch('/api/auth/session', { cache: 'no-store' }).then((res) => res.ok ? res.json() : null).then((session) => {
  ...
  const toggleTheme = () => {
    const next = !dark
    setDark(next)
    window.localStorage.setItem('alajo-theme', next ? 'dark' : 'light')
    document.documentElement.classList.toggle('dark', next)
    setAccountOpen(false)
  }

  return <>
    <div className="lg:hidden fixed top-0 inset-x-0 h-16 bg-white/50 dark:bg-[#0b2313]/50 backdrop-blur border-b border-gray-100/50 dark:border-white/10 z-[100] flex items-center justify-between px-4 safe-top">
      <button type="button" aria-label="Open navigation menu" aria-expanded={open} onClick={() => setOpen(!open)} className="w-10 h-10 rounded-lg flex items-center justify-center text-[#0b2313] dark:text-white hover:bg-gray-100/50 dark:hover:bg-white/10"><span className="text-2xl leading-none" aria-hidden="true">☰</span></button>
      <div className="flex items-center gap-2">
        <button type="button" aria-label={dark ? 'Switch to light mode' : 'Switch to dark mode'} onClick={toggleTheme} className="w-10 h-10 rounded-lg flex items-center justify-center text-[#0b2313] dark:text-white hover:bg-gray-100/50 dark:hover:bg-white/10"><span className="text-lg" aria-hidden="true">{dark ? '☀' : '☾'}</span></button>
        <div className="relative">
          <button type="button" aria-label="Open account menu" aria-expanded={accountOpen} onClick={() => setAccountOpen(!accountOpen)} className="min-w-10 h-10 px-2 rounded-full bg-[#dcefe2] dark:bg-[#183b27] text-[#0b2313] dark:text-white flex items-center justify-center font-semibold text-xs whitespace-nowrap">{firstName}</button>
          {accountOpen && <div className="absolute right-0 top-12 w-48 bg-white dark:bg-[#102719] border border-gray-100 dark:border-white/10 rounded-xl shadow-xl p-1.5 z-[120]"><Link href="/settings" onClick={() => setAccountOpen(false)} className="flex items-center gap-3 px-3 py-3 rounded-lg text-sm text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-white/5"><AlajoIcon name="settings" size={17}/> Settings</Link><Link href="/login" onClick={() => setAccountOpen(false)} className="flex items-center gap-3 px-3 py-3 rounded-lg text-sm text-gray-600 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5"><AlajoIcon name="logout" size={17}/> Logout</Link></div>}
        </div>
      </div>
    </div>
