...
create or replace function public.try_acquire_work_lock(p_lock_key text, p_owner_id uuid, p_lease_seconds integer default 30)
returns boolean
...
begin
  ...
  on conflict (lock_key) do update
    set owner_id = excluded.owner_id, acquired_at = now(), expires_at = excluded.expires_at, updated_at = now()
...
create or replace function public.release_work_lock(p_lock_key text, p_owner_id uuid)
returns boolean
