...
INSERT INTO public.market_provider_configs (market_id,provider_id,provider_type,environment,status,priority,public_config)
SELECT m.id,p.id,'PAYMENT','LIVE','ACTIVE',10,jsonb_build_object('currency','GHS')
FROM public.markets m CROSS JOIN public.provider_definitions p
WHERE m.country_code='GH' AND p.provider_key='paystack_payment'
ON CONFLICT (market_id,provider_id,environment) DO UPDATE SET status='ACTIVE',priority=10,public_config=jsonb_build_object('currency','GHS'),updated_at=now();
