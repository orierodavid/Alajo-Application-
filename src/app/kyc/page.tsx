...
export default function KycPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      if (!user) { router.replace('/login?next=/kyc'); return; }
  ...
  return (
    <main className="auth-shell">
      <section className="brand-panel">
        ...
        <p>Complete identity verification before accessing your Alajo dashboard.</p>
      ...
      <section className="auth-card">
        <div className="form-wrap">
          ...
          <p>Select <strong>either BVN or NIN</strong>. You only need one identity verification.</p>
          <div className="tabs">
          ...
          <form onSubmit={submit}>
            <label>{type.toUpperCase()}
              <input required inputMode="numeric" maxLength={KYC_DIGITS} value={value} onChange={e => setValue(e.target.value.replace(/\D/g, '').slice(0, KYC_DIGITS))} placeholder={`Enter your ${KYC_DIGITS}-digit ${type.toUpperCase()}`} />
            </label>
            <div className="notice">🔒 <strong>Development testing:</strong> use <code>{TEST_VALUES[type]}</code>. This is a synthetic test identity and must not be treated as real BVN/NIN verification.</div>
