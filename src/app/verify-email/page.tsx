...
export default function VerifyEmailPage() {
  ...
  return <main className="auth-shell"><section className="brand-panel"><div className="logo">Alajo<span>◌</span></div><h1>Smart Rotational Savings for <em>Everyone</em></h1><p>Secure your account before you begin saving with Alajo.</p><div className="features"><div>✅ <b>Secure & Verified</b><small>Verify your email before continuing.</small></div><div>👥 <b>Trusted Savings</b><small>Manage your savings groups in one place.</small></div><div>🔒 <b>Protected Account</b><small>Your account stays under your control.</small></div></div></section><section className="auth-card"><div className="form-wrap verify-wrap"><div className="auth-icon">✉️</div><h2>Verify Your Email</h2><p>Enter the 6-digit code we sent to</p><strong>{email || 'your email address'}</strong><p>Check your inbox and enter the code below.</p><div className="flex justify-center gap-2.5 mt-7" aria-label="Email verification code">{code.map((digit,index)=><input key={index} ref={element => { inputs.current[index] = element }} value={digit} onChange={event => updateDigit(index,event.target.value)} onKeyDown={event => handleKeyDown(index,event)} onPaste={handlePaste} inputMode="numeric" autoComplete={index === 0 ? 'one-time-code' : 'off'} maxLength={1} aria-label={`Verification digit ${index + 1}`} className="w-11 h-12 sm:w-12 sm:h-14 text-center text-xl font-bold border border-gray-200 rounded-lg outline-none focus:border-[#16a34a] focus:ring-2 focus:ring-green-100" /></div>{loading && <p className="mt-5 text-sm text-gray-500">Verifying…</p>}{message && <div className="mt-5 text-sm text-green-700 bg-green-50 p-3 rounded-md">{message}</div>}{error && <div className="mt-5 text-sm text-red-600 bg-red-50 p-3 rounded-md">{error}</div>}<button className="secondary-button" onClick={resend} disabled={loading || resendSeconds > 0}>{resendSeconds > 0 ? `Resend Code (${resendSeconds}s)` : 'Resend Code'}</button><Link className="back-link" href="/signup">← Back to Sign Up</Link></div></section></main>;
}
