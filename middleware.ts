...
function isUserProtected(pathname: string) { return USER_PROTECTED_PREFIXES.some((prefix) => pathname === prefix || pathname.startsWith(`${prefix}/`)) }
...
function privateResponseHeaders(response: NextResponse) { securityHeaders(response); response.headers.set('X-Robots-Tag', 'noindex, nofollow, noarchive'); response.headers.set('Cache-Control', 'private, no-store, max-age=0'); return response }
function edgeAbuseGuard(request: NextRequest) {
  ...
  const now = Date.now(), current = edgeBuckets.get(key)
  if (!current || current.resetAt <= now) { edgeBuckets.set(key, { count: 1, resetAt: now + 60_000 }); return null }
  if (current.count >= 120) return new NextResponse(JSON.stringify({ error: 'TOO_MANY_REQUESTS' }), { status: 429, headers: { 'Content-Type': 'application/json', 'Retry-After': String(Math.max(1, Math.ceil((current.resetAt - now) / 1000))), 'Cache-Control': 'no-store' } })
  current.count++
  return null
}
...
export async function middleware(request: NextRequest) {
  ...
  if (isAdmin) { const { data: role, error: roleError } = await supabase.rpc('get_my_admin_role'); if (roleError || !role) return securityHeaders(NextResponse.redirect(new URL('/dashboard', request.url))); return privateResponseHeaders(response) }
  const { data: accessState, error: accessError } = await supabase.rpc('get_my_access_state')
  if (accessError || !accessState?.profile_exists) { await supabase.auth.signOut(); return securityHeaders(NextResponse.redirect(new URL('/login?error=account_not_found', request.url))) }
  const profileStatus = String(accessState.profile_status ?? '').toLowerCase()
  if (profileStatus && !['active', 'pending'].includes(profileStatus)) { await supabase.auth.signOut(); return securityHeaders(NextResponse.redirect(new URL('/login?error=account_inactive', request.url))) }
  const kycVerified = accessState.kyc_verified === true, dvaActive = accessState.virtual_account_active === true, verificationComplete = accessState.verification_complete === true && kycVerified && dvaActive
  if (!verificationComplete) { const target = kycVerified ? '/kyc/status' : (accessState.kyc_status ? '/kyc/status' : '/kyc'); if (pathname !== target) return securityHeaders(NextResponse.redirect(new URL(target, request.url))) }
  return privateResponseHeaders(response)
}
export const config = { matcher: ['/api/:path*', '/admin/:path*', '/dashboard/:path*', '/groups/:path*', '/join-group/:path*', '/contributions/:path*', '/payouts/:path*', '/wallet/:path*', '/transactions/:path*', '/notifications/:path*', '/settings/:path*', '/help-center/:path*', '/onboarding/:path*'] }
