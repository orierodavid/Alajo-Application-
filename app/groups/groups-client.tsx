    ...
    <main className="flex-1 min-w-0">
      ...
      <section className="p-5 lg:p-8 max-w-7xl mx-auto">
        ...
        <div className="mt-5 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {visibleGroups.map((group) => { const joined = myGroupIds.has(group.id); const payout = Number(group.contribution_amount) * group.slot_count; const isFull = group.status === 'full' || group.memberCount >= group.slot_count; return <article key={group.id} className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h4 className="font-bold text-gray-900 text-[16px]">{group.name}</h4><p className="text-xs text-gray-400 mt-1">{group.description}</p></div><span className={`text-xs px-2 py-1 rounded-full ${isFull ? 'bg-gray-100 text-gray-500' : joined ? 'bg-green-50 text-green-600' : 'bg-green-50 text-green-600'}`}>{isFull ? 'Full' : joined ? 'Active' : 'Open'}</span></div><div className="mt-3 space-y-2 text-sm"><p><span className="text-gray-400">Contribution:</span> <b>{money(Number(group.contribution_amount))}</b></p><p><span className="text-gray-400">Cycle:</span> <b>{cycleLabel(group.cycle)}</b></p><p><span className="text-gray-400">Members:</span> <b>{group.memberCount}/{group.slot_count}</b></p><p><span className="text-gray-400">Payout:</span> <b>{money(payout)}</b></p><p><span className="text-gray-400">Status:</span> <b className="text-green-600">{joined ? 'Active' : isFull ? 'Full' : 'Available'}</b></p></div>{joined ? <div className="mt-4 rounded-lg bg-green-50 text-green-700 px-3 py-2 text-sm font-medium flex items-center gap-2"><AlajoIcon name="check" size={16}/>You are a member of this group.</div> : tab === 'available' ? <Link href={isFull ? '#' : `/join-group/${group.id}`} aria-disabled={isFull} className={`mt-4 flex items-center justify-center gap-2 w-full text-center bg-[#14532d] hover:bg-[#123f24] ${isFull ? 'pointer-events-none opacity-50' : ''} text-white font-semibold rounded-lg py-2.5`}>Join Group <AlajoIcon name="arrow-up" size={15}/></Link> : null}</article> })}
        </div>
