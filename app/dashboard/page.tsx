...
function Icon({ name, size = 18 }: { name: IconName; size?: number }) {
  const paths: Record<IconName, string> = {
    ...
    coin: 'M20 12c0 3-3.6 5-8 5s-8-2-8-5 3.6-5 8-5 8 2 8 5 8 5ZM4 12v4c0 3 3.6 5 8 5s8-2 8-5v-4',
  ...
  useEffect(() => {
    fetch('/api/auth/session', { cache: 'no-store' })
      .then(async (response) => {
        ...
        if (data.authenticated && data.name) setName(data.name)
      })
      .catch(() => {})
  }, [])

  const cards: Array<[IconName, string, string, string, string, string]> = [
    ['wallet', 'bg-green-50', 'text-[#16a34a]', 'Wallet Balance', money(walletBalance), 'Available Balance'],
  ...
  return (
    <div className="min-h-screen bg-[#f8faf9] text-gray-900 flex">
      <aside className="hidden lg:flex w-[250px] shrink-0 min-h-screen bg-[#0b2313] text-white p-5 flex-col fixed inset-y-0 left-0">
        <div className="px-2 text-[26px] font-extrabold tracking-tight flex items-center gap-2">
          Alajo <span className="text-yellow-400"><Icon name="coin" size={20} /></span>
        </div>
        <nav className="mt-8 flex-1 space-y-1 text-[14px] font-medium">
          {navItems.map(([icon, label, href]) => (
            <Link
              key={label}
              href={href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg ${label === 'Dashboard' ? 'bg-white/10 text-white' : 'text-gray-300 hover:bg-white/5'}`}
            >
              <Icon name={icon} />
              <span>{label}</span>
          ...
          <Link href="/login" className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-gray-300 hover:bg-white/5">
            <Icon name="logout" />
            <span>Logout</span>
          </Link>
        </nav>
        <div className="bg-[#123524] rounded-xl p-4 text-white">
          ...
          <Link href="/invite-earn" className="mt-3 inline-block bg-white text-[#0b2313] text-[13px] font-semibold px-3 py-1.5 rounded-md">
            Invite Friends
          </Link>
        </div>
      </aside>
          ...
          <div className="mt-6 grid grid-cols-1 xl:grid-cols-2 gap-5">
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <div className="flex items-center justify-between">
                ...
                <Link href="/transactions" className="text-[#16a34a] text-[13px] font-medium">View All</Link>
              </div>
              <div className="mt-5 text-sm text-gray-500">Your recent savings activity will appear here.</div>
            </div>

            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <div className="flex items-center justify-between">
                <h2 className="font-semibold">My Active Groups</h2>
                <Link href="/groups" className="text-[#16a34a] text-[13px] font-medium">View All Groups</Link>
              </div>
              <div className="mt-5 text-sm text-gray-500">Open Groups to view your savings groups.</div>
            </div>
          </div>
        </section>
      </main>
