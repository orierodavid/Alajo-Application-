...
export default function WalletSuccessPage() {
  ...
  const title = success ? 'Wallet funded successfully' : pending ? 'Payment is still processing' : 'Wallet funding was not completed'
    ...
    <div className="min-h-screen bg-[#f7f8f9] text-gray-900 flex items-center justify-center p-5">
      <main className="w-full max-w-md">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-7 text-center">
          <div className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center ${success ? 'bg-green-50 text-[#16a34a]' : pending ? 'bg-amber-50 text-amber-600' : 'bg-red-50 text-red-600'}`}>
          ...
          <div className="mt-6 rounded-xl bg-gray-50 p-4 text-left">
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-[12px] text-gray-500">Payment status</p><p className={`text-sm font-semibold ${success ? 'text-[#16a34a]' : pending ? 'text-amber-600' : 'text-red-600'}`}>{success ? 'Verified' : pending ? 'Processing' : 'Not credited'}</p></div>
            </div>
          </div>
