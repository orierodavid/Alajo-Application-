...
async function requireAdmin() {
  ...
  if (!role) return null
  return { user, admin: createAdminClient(), caller: supabase }
}
...
export async function GET() {
  try {
    ...
    const profileMap = new Map((profiles || []).map((p: any) => [p.id, p])); const memberMap = new Map((members || []).map((m: any) => [m.id, m])); const groupMap = new Map((groups || []).map((g: any) => [g.id, g]))
    const rows = (cases || []).map((c: any) => { const m = memberMap.get(c.group_member_id); const p = m ? profileMap.get(m.user_id) : null; const g = groupMap.get(c.group_id); return { ...c, user: { id: m?.user_id || null, name: p?.full_name || 'User', email: p?.email || '' }, group: g ? { id: g.id, name: g.name, cycle: g.cycle, finalizedMemberCount: g.finalized_member_count, status: g.status } : null, payoutPosition: m?.payout_position ?? null, payoutReceivedAt: m?.payout_received_at ?? null, membershipStatus: m?.status ?? null } })
...
export async function POST() {
  try {
    ...
    const { data: result, error } = await auth.caller.rpc('process_default_recovery')
    if (error) throw error
    const { data: protection, error: protectionError } = await auth.caller.rpc('protect_due_payouts')
    ...
    if (memberIds.length) {
      ...
      const profileMap = new Map((profiles || []).map((p: any) => [p.id, p])); const memberMap = new Map((members || []).map((m: any) => [m.id, m]))
      for (const c of newCases || []) {
        ...
        const emailResult = await sendEmail({
          ...
          text: postPayout ? 'Your contribution remains unpaid after the 30-day recovery window. Your savings group continues; the outstanding amount is now in recovery.' : 'Your contribution remains unpaid after the 30-day recovery window. Because you have not received your payout, your membership has been removed from the active cycle and the group cycle has been recalculated.',
