...
export async function GET() {
  try {
    ...
    const { data: account, error } = await supabase.from('user_virtual_accounts').select('account_number,account_name,bank_name,currency,status').eq('user_id', user.id).eq('currency', 'NGN').limit(1).maybeSingle()
