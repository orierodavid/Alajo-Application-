...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      const { data: marked, error: markError } = await supabase.rpc('mark_payout_processing', { p_payout_request_id: (payout as any).id, p_provider_reference: providerReference })
      ...
      await supabase.rpc('fail_payout_atomic', { p_payout_request_id: (payout as any).id, p_failure_code: 'PROVIDER_TRANSFER_FAILED', p_failure_reason: reason })
