...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: schedule, error: scheduleError } = await supabase.from('contribution_schedules').select('id,group_member_id,status').eq('id',scheduleId).maybeSingle()
    if (scheduleError) throw scheduleError; if (!schedule) return errorResponse('CONTRIBUTION_NOT_FOUND',404)
    ...
    if (membershipError) throw membershipError; if (!membership) return errorResponse('You do not have access to this contribution.',403)
    const { data, error: rpcError } = await supabase.rpc('pay_contribution_from_wallet',{p_schedule_id:scheduleId,p_idempotency_key:key})
    if (rpcError) { console.error('Contribution wallet RPC error:',rpcError); return errorResponse('Payment could not be completed.',400) }
