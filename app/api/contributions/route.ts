...
export async function GET() {
  try {
    ...
    const rows = (schedules ?? []).map((schedule) => {
      ...
      const currentMonth = isCurrentMonth(schedule.due_date)
      return {
        id: schedule.id,
        ...
        canPay: currentMonth && PAYABLE_STATUSES.includes(schedule.status),
    ...
    const completed = rows.filter((row) => completedStatuses.includes(row.status)); const missed = rows.filter((row) => missedStatuses.includes(row.status)); const upcoming = rows.filter((row) => { const due = new Date(`${row.dueDate.slice(0, 10)}T00:00:00Z`); return upcomingStatuses.includes(row.status) && due >= today && due <= sevenDays })
