...
export async function POST(request: Request) {
  try {
    ...
    const [{ data: profile, error: profileError }, { data: userKyc }] = await Promise.all([
      admin.from('profiles').select('full_name, phone, email, country_code').eq('id', user.id).single(),
      admin.from('user_kyc_profiles').select('country_code').eq('user_id', user.id).maybeSingle(),
    ])
    ...
    const { data: rawKycConfig, error: kycConfigError } = await admin.from('market_provider_configs')
      .select('id, provider_definitions!inner(provider_key, provider_type, status, capabilities)')
    ...
    // The installed Paystack identity adapter is Nigeria-specific. Other markets can be enabled in Admin only after a compatible KYC adapter is installed.
