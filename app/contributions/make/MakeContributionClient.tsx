...
function MakeContributionContent(){
 ...
 useEffect(()=>{let dead=false;async function load(){if(!id){setLoading(false);return}try{const {createClient}=await import('@/lib/supabase/client');const supabase=createClient();const {data,error}=await supabase.from('contribution_schedules').select('id,amount,due_date,status').eq('id',id).maybeSingle();if(!error&&data&&!dead){setAmount(Number(data.amount||0));setDueDate(data.due_date||'')}}finally{if(!dead)setLoading(false)}}load();return()=>{dead=true}},[id])
...
export default function MakeContributionClient(){return <Suspense fallback={<main className="min-h-screen bg-[#f8faf9] flex items-center justify-center text-gray-500">Loading contribution…</main>}><MakeContributionContent/></Suspense>}
