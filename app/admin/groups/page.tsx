...
export default async function AdminGroupsPage() {
  ...
  return <section className="p-5 sm:p-8 max-w-6xl mx-auto text-white">
    ...
    <div className="mt-8 grid gap-4">
      {groups?.length ? groups.map(group => <article key={group.id} className="rounded-2xl border border-[#244332] bg-[#0d1d13] p-5 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4"><div><div className="flex items-center gap-2"><h2 className="text-lg font-bold text-white">{group.name}</h2><span className="rounded-full bg-[#123b22] px-2.5 py-1 text-xs font-semibold text-[#61e58d]">{group.status}</span></div><p className="text-sm text-[#b7c7be] mt-1">{group.description || 'Structured rotational savings group.'}</p></div><div className="text-sm font-semibold text-white">₦{Number(group.contribution_amount).toLocaleString()} / month</div></div>
        <div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm"><div><span className="text-[#9fb3a8] block">Cycle</span><strong className="text-white">{group.cycle === 'five_month' ? '5 months' : '10 months'}</strong></div><div><span className="text-[#9fb3a8] block">Slots</span><strong className="text-white">{group.slot_count}</strong></div><div><span className="text-[#9fb3a8] block">Starts</span><strong className="text-white">{group.start_date || '—'}</strong></div><div><span className="text-[#9fb3a8] block">Finishes</span><strong className="text-white">{group.finish_date || '—'}</strong></div></div>
