...
export default function CreateGroupPage() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      const response = await fetch('/api/admin/groups', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, description, contribution_amount: Number(amount), cycle: Number(cycle), slot_count: Number(slots) }) })
  ...
  return (
    <section className="p-5 sm:p-8 max-w-4xl mx-auto">
      ...
      <div className="mt-2"><h1 className="text-3xl font-bold">Create group</h1><p className="text-gray-500 dark:text-gray-400 mt-2">Set the rules once. Alajo automatically starts the next cycle on the 1st and closes it on the 29th.</p></div>
      <form onSubmit={submit} className="alajo-glass mt-8 rounded-2xl p-5 sm:p-8 space-y-6">
        ...
        <div className="alajo-glass rounded-xl p-4 text-sm"><p className="font-semibold">Automatic schedule</p><p className="text-gray-500 dark:text-gray-400 mt-1">The next cycle starts on the 1st of the next month. Each monthly contribution is payable through the 29th. The group finish date is the 29th of the final cycle month.</p></div>
