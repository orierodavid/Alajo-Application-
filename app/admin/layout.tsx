...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // /admin/login is intentionally outside the protected application shell.
  // Never render admin navigation, avatar, or logout controls until an
  // authenticated admin session has been established.
  ...
  return <div className="min-h-screen bg-[#f5faf7] text-gray-900 dark:bg-[#07150c] dark:text-white lg:flex">
    <aside className="hidden lg:flex lg:w-64 lg:flex-col lg:fixed lg:inset-y-0 border-r border-gray-200 dark:border-white/10 bg-white dark:bg-[#0b2113]">
      ...
      <div className="p-4 border-t border-gray-100 dark:border-white/10"><p className="text-xs text-gray-400 truncate mb-2">{user.email || 'Administrator'}</p><p className="text-[11px] uppercase tracking-wider text-[#16a34a] mb-3">{role}</p><AdminLogout /></div>
    </aside>
